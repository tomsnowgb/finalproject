...
%    fuse saved as tiff format
%
% The following fields are returned in the table RESULT
%
%
% Auto-generated by imageBatchProcessor app on 18-Apr-2022
%----------------------------------------------------------
...
if(nargin<1)
    inDir = 'C:\Users\tomsn\OneDrive\Documents\University\Final Project\Final_Work';
else
...
for imgInd = 1:numel(imds.Files)
    ...
    try
        ...
        % Process
        if (contains(outImageFile,'SatData'))
            oneResult = imfcSat(im);
        else
            if (contains(outImageFile,'Risk'))
                oneResult = imfcRisk(im);
            else
                oneResult = imfcElv(im);
            end
        end
        ...
        if(~isempty(outDir))
            ...
            for ind = 1:numel(fileFieldsAndFormat)
                ...
                x = regexp(outImageFile, '\d*', 'match');
                if (contains(outImageFile,'SatData'))
                    outImageFileWithExtension = ['Images\',x{1}, fieldName, '.', fileFormat];
                else 
                    if (contains(outImageFile,'Risk'))
                        outImageFileWithExtension = ['Labels\',x{1},  fieldName, '.', fileFormat];
                    else 
                        outImageFileWithExtension = ['Heights\',x{1},  fieldName,  '.', fileFormat];
                    end
                end
